...
#include "bsp_nearair.h"//near add
...
/*  uint8_t *inDataBuffer;
  ...
  while(1) {
    Main_Menu(airContext);//Near add
/*    recvlen = _uart_get_one_packet(inDataBuffer, UART_ONE_PACKAGE_LENGTH);
    if (recvlen <= 0)
      continue; 
//    sppUartCommandProcess(inDataBuffer, recvlen, Context);
    uart_recv_log("Near> %s",inDataBuffer);//near add
    memset(inDataBuffer,0x0,UART_ONE_PACKAGE_LENGTH);//near add*/
  }  
//exit:
