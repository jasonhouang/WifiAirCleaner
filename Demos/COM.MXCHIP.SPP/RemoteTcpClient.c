...
//static nearair_Context_t *airContent = NULL;//near add
...
//  int currentRecved = 0;//near add
  ...
  while(1) {   
    if(remoteTcpClient_fd == -1 ) {
      ...
      Context->appStatus.isRemoteConnected = true;//replace:set_network_state(REMOTE_CONNECT, 1);//
      client_log("Remote server connected at port: %d, fd: %d",  Context->flashContentInRam.appConfig.remoteServerPort,
                 remoteTcpClient_fd);
    
      nearair_login(inContext);//near add
      ...
      /*recv UART and NearAir data using loopback fd*/
      ...
      if (FD_ISSET(remoteTcpClient_fd, &readfds)) {
        ...
        if(len <= 0) {
          ...
          Context->appStatus.isRemoteConnected = false;//repalce:set_network_state(REMOTE_CONNECT, 0);//
          goto ReConnWithDelay;
        }
        //currentRecved += len;//near add
        nearairWlanCommandProcess(inDataBuffer,&len,remoteTcpClient_fd,Context,airContext);//currentRecved->len        
      }
      
      if(false == Context->appStatus.isRemoteConnected)
          goto ReConnWithDelay;      
    }
    Continue:    
      continue;
